...
public:
    ...
    void InitSpannableString();
    void SetText(const char* inputText);
    void InitIsSizeSpan(int16_t characterSize);
    bool SetIsSizeSpan(bool value, uint16_t startIndex, uint16_t endIndex);
    bool GetIsSizeSpan(uint16_t index);
    ...
    bool GetHeight(uint16_t index, uint16_t& outputHeight);

private:
    uint16_t isSizeSpanLen_;

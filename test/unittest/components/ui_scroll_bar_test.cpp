...
public:
    ...
    int16_t GetRadius(){return radius_;}
    Style* GetBackStyle(){return backgroundStyle_;}
    Style* GetForeStyle(){return foregroundStyle_;}
    float GetProportion(){return foregroundProportion_;}
    uint8_t GetOpacity(){return opacity_;}
};

class TestBoxScrollBarTest : public UIBoxScrollBar {
public:
    int16_t GetWidth(){return backGround_.GetWidth();}
    int16_t GetHeight(){return backGround_.GetHeight();}
    Style* GetBackStyle(){return backgroundStyle_;}
    Style* GetForeStyle(){return foregroundStyle_;}
    float GetProportion(){return foregroundProportion_;}
    uint8_t GetOpacity(){return opacity_;}
...
HWTEST_F(ScrollBarTest, UIScrollBarSetOpacity, TestSize.Level0)
{
    ...
    for (int8_t i = 0; i < sizeof(opa) / sizeof(uint8_t); i++) {
        ...
        boxBar.SetForegroundProportion(opa[i]);

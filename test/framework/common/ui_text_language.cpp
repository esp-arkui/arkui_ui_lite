...
void BitmapFontInit()
{
    UIFontInitialization* uiFontInitialization = UIFontInitialization::GetInstance();
    if (uiFontInitialization == nullptr) {
        return;
    }
    uiFontInitialization->SetTextLangFontsTable(g_uiTextLangFontsTable, LANGUAGE_FONT_ID_MAX);
    uiFontInitialization->SetLangTextDefaultParamTable(g_langTextDefaultParamTable, LANGUAGE_MAX);
    uiFontInitialization->SetMaxTextId(LANGUAGE_TEXT_ID_MAX);
}
} // namespace OHOS

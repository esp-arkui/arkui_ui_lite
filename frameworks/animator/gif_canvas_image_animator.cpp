...
void GifCanvasImageAnimator::Callback(UIView* view)
{
    ...
    Rect curViewRect;
    Rect orRect = view->GetMaskedRect();
    int16_t top = orRect.GetTop();
    int16_t bottom = orRect.GetBottom() - 1;
    if (bottom <= 0 || top >= (Screen::GetInstance().GetHeight() - 1)) {
        if ((view->GetViewType() == UI_CANVAS)) {
            this->Stop();
            return;
        }
    }
    ...
    if (curTime != 0) {
        uint32_t deltaTimeTem = curTime + deltaTime_ - lastRunTime_;
        if (deltaTimeTem >= delayTime_) {
        deltaTime_ = deltaTimeTem - delayTime_;
                                              ...
                                              const ColorMapObject* colorMap) const
{
    ...
    for (int32_t x = 0; x < gifFileType->SHeight; x++) {
        for (int32_t y = 0; y < gifFileType->SWidth; y++) {
            DealGifImageLineData(gifImageDesc, savedImage, gcb, colorMap,
                                 transparentColor, x, y, colorIndex, index);
        }
    }
}

void GifCanvasImageAnimator::DealGifImageLineData(const GifImageDesc* gifImageDesc,
                                                  const SavedImage* savedImage,
                                                  GraphicsControlBlock& gcb,
                                                  ...
                                                  uint32_t& index) const
{
    ...
    if ((heightVale >= gifImageDesc->Top) && (heightVale < gifImageDesc->Top + gifImageDesc->Height) &&
        (widthVale >= gifImageDesc->Left) && (widthVale < gifImageDesc->Left + gifImageDesc->Width)) {
        int32_t loc = 0;
        loc = (heightVale - gifImageDesc->Top) * gifImageDesc->Width + (widthVale - gifImageDesc->Left);

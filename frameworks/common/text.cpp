...
SpannableString* Text::CreateSpannableString()
{
    ...
    if (spannableString != nullptr) {
        spannableString->InitSpannableString();
    } else {
        return nullptr;
    }
                ...
                OpacityType opaScale)
{
    ...
    for (uint16_t i = 0; i < lineCount; i++) {
        ...
        if ((style.lineSpace_ == 0) && (spannableString_ != nullptr)) {
            curLineHeight = font->GetLineMaxHeight(&text_[lineBegin], textLine_[i].lineBytes, fontId_, fontSize_,
...
uint16_t Text::GetLine(int16_t width, uint8_t letterSpace, uint16_t ellipsisIndex, uint32_t& maxLineBytes)
{
    ...
    while ((begin < textLen) && (text_[begin] != '\0') && (lineNum < MAX_LINE_COUNT)) {
        begin += GetTextLine(begin, textLen, width, lineNum, letterSpace, letterIndex, spannableString_);
...
uint32_t Text::GetTextLine(uint32_t begin,
                           uint32_t textLen,
                           int16_t width,
                           uint16_t lineNum,
                           uint8_t letterSpace,
                                    ...
                                    uint16_t beginIndex, uint16_t count)
{
    ...
    Point textSize = TypedText::GetTextSize(preText.substr(beginIndex, count).c_str(), fontId_, fontSize_,
        style.letterSpace_, style.lineHeight_, maxWidth, style.lineSpace_, spannableString_);
    return textSize.x;
}
...
void Text::SetAbsoluteSizeSpan(uint16_t start, uint16_t end, uint8_t size)
{
    ...
    if (spannableString_ != nullptr && start <= characterSize_) {
        for (uint16_t i = start; i < end && i < characterSize_; i++) {
            spannableString_->SetIsSizeSpan(true, i, i+1);
            spannableString_->SetFontSize(size, i, i + 1);
            spannableString_->SetFontSize(fontId, i, i + 1);
        }
    }
}

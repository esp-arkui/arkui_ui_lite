...
bool SpannableString::SetSpannableString(const SpannableString* input)
{
    ...
    // 加入新的
    SetText(input->text_);
    isSizeSpanLen_= input->isSizeSpanLen_;
    bool hasInit = InitIsSizeSpan(isSizeSpanLen_);
    hasSet = hasSet & hasInit;
    if(hasInit)
    {
        for(uint32_t i=0;i<isSizeSpanLen_;i++)
        {
            isSizeSpan_[i]= input->isSizeSpan_[i];
        }
...
void SpannableString::SetText(const char* inputText)
{
    ...
    bool success = InitIsSizeSpan(textLen);
    if(!success)
    {
        UIFree(text_);
        text_ = nullptr;
    ...
    // fill isSizeSpan with 4 List
    bool hasFill = FillIsSizeSpan();
    if(!hasFill)
    {
        UIFree(text_);
        text_ = nullptr;
...
bool SpannableString::FillIsSizeSpan()
{
    bool success = true;
    if (isSizeSpan_ != nullptr) {
        if (spanList_.Size() > 0) {
...
void SpannableString::FillWithSpan()
{
    ...
    for (; tempSpan != sizeList_.End(); tempSpan = tempSpan->next_) {
        ...
        if (tempStart < isSizeSpanLen_) {
            tempEnd = (tempEnd < isSizeSpanLen_) ? tempEnd : isSizeSpanLen_;
...
bool SpannableString::InitIsSizeSpan(uint32_t characterSize)
{
    UIFree(isSizeSpan_);

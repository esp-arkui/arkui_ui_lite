...
void ClipUtils::PerformScan(const ClipPath& path, const ImageInfo* imageInfo)
{
    ...
    GraphicGeometryScanline scanline;
    ...
    using PathTransform = OHOS::GraphicDepictTransform<UICanvasPath>;
    ...
    while (rasterizer.SweepScanline(scanline)) {
        ...
        typename GraphicGeometryScanline::ConstIterator span = scanline.Begin();
        int16_t index = 0;
        while (true) {
            int x = span->x;

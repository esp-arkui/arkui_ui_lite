    ...
    for (int16_t y = in.yMin; y <= in.yMax; y++) {
        ...
        for (int16_t x = xMin; x <= xMax; x++) {
            ...
            if ((u >= 0) && (intU < in.info.header.width - 1) && (v >= 0) && (intV < in.info.header.height - 1)) {
                ...
                Color16 result;
#if ENABLE_FIXED_POINT
                result.red = static_cast<uint8_t>(outR >> FIXED_Q_NUM);   // 15: shift 15 bit right to convert fixed to int
                result.green = static_cast<uint8_t>(outG >> FIXED_Q_NUM); // 15: shift 15 bit right to convert fixed to int
                result.blue = static_cast<uint8_t>(outB >> FIXED_Q_NUM);  // 15: shift 15 bit right to convert fixed to int
#else

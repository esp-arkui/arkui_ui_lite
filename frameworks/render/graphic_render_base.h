...
#define BLEND_FROM_WHILE \
    while (rect.y2 > 0) { \
        ...
        if (rw.ptr) { \
            ...
            if (len > 0) { \
                ...
                if (len > 0) { \
                    pixfmtType_->BlendFrom(src, x1dst, rdst.y1, x1src, rsrc.y1, len, cover); \
                } \
            } \
        } \
        rdst.y1 += incy; \
    ...
    class GraphicRenderBase {
    public:
        typedef PixelFormat PixfmtType;
        typedef typename PixfmtType::ColorType ColorType;
        typedef typename PixfmtType::RowData RowData;

        GraphicRenderBase() : pixfmtType_(0), clipBox_(1, 1, 0, 0) {}

        explicit GraphicRenderBase(PixfmtType& ren)
            : pixfmtType_(&ren), clipBox_(0, 0, ren.Width() - 1, ren.Height() - 1) {}
        ...
        void Clear(const ColorType& color)
        {
            ...
            if (Width()) {
                for (y = 0; y < Height(); y++) {
                    pixfmtType_->CopyHline(0, y, Width(), color);
                }
            }
        }
        ...
        void ReplaceColor(const ColorType& color)
        {
            ...
            if (Width()) {
                for (y = 0; y < Height(); y++) {
                    pixfmtType_->ReplaceHlineColor(0, y, Width(), color);
                }
            }
        }
        ...
        void Fill(const ColorType& color)
        {
            ...
            if (Width()) {
                for (y = 0; y < Height(); y++) {
                    pixfmtType_->blend_hline(0, y, Width(), color, COVER_MASK);
                }
            }
        }
        ...
        {
            return Inbox(x, y) ? pixfmtType_->pixel(x, y) : ColorType::no_color();
        }
         ...
         */
        void BlendHline(int x1, int y, int x2, const ColorType& color, uint8_t cover)
        {
            if (x1 > x2) {
                int t = x2;
                x2 = x1;
         ...
         * @param colors Scan line corresponding color array.
         */
        void BlendSolidHspan(int x, int y, int len, const ColorType& color, const uint8_t* covers)
        {
         ...
         * @param colors Scan line corresponding color array.
         */
        void CopyColorHspan(int x, int y, int len, const ColorType* colors)
        {
            if (y > GetYmax() || y < GetYmin()) {
                return;
            }
         ...
         */
        void BlendColorHspan(int x, int y, int len, const ColorType* colors, const uint8_t* covers,

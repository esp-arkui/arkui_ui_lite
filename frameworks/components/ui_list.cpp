...
bool UIList::OnDragEndEvent(const DragEvent& event)
{
    ...
    if (!DragThrowAnimator(current, last, event.GetDragDirection(), dragBack_, selectPosition_)) {
...
void UIList::FixDistance(int16_t& distanceX, int16_t& distanceY, uint16_t selectPosition)
{
    ...
    if (direction_ == VERTICAL) {
        FixVerDistance(distanceY, selectPosition);
    } else {
        FixHorDistance(distanceX, selectPosition);
    }
}

void UIList::FixHorDistance(int16_t& distanceX, uint16_t selectPosition)
{
    ...
    while (targetView != nullptr) {
        ...
        if (pos <= selectPosition && pos + width >= selectPosition) {
            int16_t offset = selectPosition - (pos + width / 2); // 2 : half of width
...
void UIList::FixVerDistance(int16_t& distanceY, uint16_t selectPosition)
{
    UIView* targetView = childrenHead_;
    while (targetView != nullptr) {
        int16_t pos = targetView->GetY();
        int16_t height = targetView->GetRelativeRect().GetHeight();
        if (pos <= selectPosition && pos + height >= selectPosition) {
            int16_t offset = selectPosition - (pos + height / 2); // 2 : half of height

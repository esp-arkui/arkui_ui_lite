                                          ...
                                          SpannableString* spannableString)
{
    ...
    if (spannableString != nullptr && spannableString->GetSpannable(letterIndex)) {
        ...
        spannableString->GetHeight(letterIndex, height);
        if (height == 0) {
            uint16_t tempHeight = 0;
            tempHeight = fontEngine->GetHeight(tempFontId, tempSize);
            height = static_cast<int16_t>(tempHeight);
            spannableString->SetHeight(height, letterIndex, letterIndex + 1);
        } else {
            /* height is the output of GetHeight() */
        }
        return width;
    } else {
        uint16_t tempHeight = 0;
        tempHeight = fontEngine->GetHeight(fontId, fontSize);
        height = static_cast<int16_t>(tempHeight);

...
{
    SetBaseFontId(UIFontInitialization::GetInstance()->GetBitmapFontIdMax());
...
#else
    UITextLanguageFontParam* fontParam = UIFontInitialization::GetInstance()->GetTextLangFontsTable(fontId);
...
{
    if (ttfName == nullptr) {
        return UIFontInitialization::GetInstance()->GetBitmapFontIdMax();
    }
    uint8_t id;
    for (id = 0; id < UIFontInitialization::GetInstance()->GetBitmapFontIdMax(); ++id) {
        UITextLanguageFontParam* fontParam = UIFontInitialization::GetInstance()->GetTextLangFontsTable(id);
        if (fontParam != nullptr) {
...
{
    return UIFontInitialization::GetInstance()->GetTextLangFontsTable(fontId);
}

...
GlyphsManager::GlyphsManager()
    : binHeader_{{0}},
      ...
      isFontIdSet_(false)
{
    fontId_ = UIFontInitialization::GetInstance()->GetBitmapFontIdMax();
}
GlyphsManager::~GlyphsManager() {}
...
int8_t GlyphsManager::SetFile(int32_t fp, uint32_t start)
{
    ...
    if (binHeader_.fontNum > UIFontInitialization::GetInstance()->GetBitmapFontIdMax()) {
        return INVALID_RET_VALUE;
    }
...
int8_t GlyphsManager::SetCurrentFontId(uint8_t fontId)
{
    ...
    if (fontId > UIFontInitialization::GetInstance()->GetBitmapFontIdMax()) {
        return INVALID_RET_VALUE;
    }
    if (fontId_ == fontId) {
        return RET_VALUE_OK;

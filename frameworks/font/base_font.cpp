...
{
    if ((langId >= UIFontInitialization::GetInstance()->GetTotalLangId()) || (pParam == nullptr)) {
        return INVALID_RET_VALUE;
    }

    LangTextParam *pTable = UIFontInitialization::GetInstance()->GetLangTextDefaultParamTable();
    if (pTable == nullptr) {
        return INVALID_RET_VALUE;
    }
